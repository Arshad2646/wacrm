...
describe('validateStepsForActivation', () => {
  ...
  it('validates webhook URLs', () => {
    ...
    const wrongProtocol = validateStepsForActivation([
      {
        step_type: 'send_webhook',
        step_config: { url: 'http://hooks.example.com' },
      },
    ]);
    ...
    const localhost = validateStepsForActivation([
      {
        step_type: 'send_webhook',
        step_config: { url: 'https://localhost/hook' },
      },
    ]);
    expect(localhost.map((i) => i.message)).toContain(
