...
/**
 * Owner / admin: change the account-wide bot on/off switch. Agents can
 * still pause/resume AI per conversation through the inbox, but the
 * global switch affects the whole business and stays admin-scoped.
 */
export function canManageAccountBot(role: AccountRole): boolean {
  return hasMinRole(role, 'admin');
}
