...
function logDbError(context: string, error: unknown) {
  console.error(`[flows] ${context}:`, error);
}
...
> {
  try {
    const ctx = await requireAdvancedCrmTools(minRole);
    ...
    return { ok: true, userId: ctx.userId, accountId: ctx.accountId };
  } catch (error) {
...
export async function GET(
  ...
  const guard = await requireOwnership(id, 'viewer');
...
export async function PUT(
  ...
  if (body.nodes !== undefined) {
    ...
    const { error: delErr } = await admin
      ...
      .eq('flow_id', id);
    if (delErr) {
      logDbError('node delete failed', delErr);
      return NextResponse.json(
        { error: 'Failed to replace flow nodes' },
        { status: 500 }
      );
    }
    if (body.nodes.length > 0) {
      ...
      if (insErr) {
        logDbError('node insert failed', insErr);
        return NextResponse.json(
