...
import { handleInboundAiReply } from '@/lib/ai/whatsapp-bot';
import {
...
async function processMessage(
  ...
  const { data: inboundMessageRecord, error: msgError } = await supabaseAdmin()
    .from('messages')
    .insert({
  ...
  if (!flowConsumed && contentText && inboundMessageRecord?.id) {
    await handleInboundAiReply({
      ...
      inboundMessageId: inboundMessageRecord.id,
