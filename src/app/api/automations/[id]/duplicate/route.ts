...
export async function POST(
  ...
  try {
    ...
    const { data: original, error: origErr } = await admin
      ...
      .maybeSingle();
    if (origErr) {
      logDbError('duplicate source lookup failed', origErr);
    ...
    const { data: copy, error: copyErr } = await admin
      ...
      .single();
    if (copyErr || !copy) {
      logDbError('duplicate create failed', copyErr);
      return NextResponse.json(
    ...
    if (steps && steps.length > 0) {
      ...
      const { error: insErr } = await admin
        ...
        .insert(rows);
      if (insErr) {
        logDbError('duplicate steps failed', insErr);
        return NextResponse.json(
          { error: 'Failed to duplicate automation steps' },
          { status: 500 }
        );
      }
    }
