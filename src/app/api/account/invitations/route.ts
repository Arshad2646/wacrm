...
import { accountAllowsMultipleUsers } from '@/lib/saas/packages';
import {
  checkRateLimit,
  rateLimitResponse,
  RATE_LIMITS,
...
export async function POST(request: Request) {
  try {
    ...
    if (accountError || !account) {
      console.error(
        '[POST /api/account/invitations] account lookup error:',
        accountError
      );
      return NextResponse.json(
        { error: 'Failed to verify account package' },
        { status: 500 }
      );
    }

    if (!accountAllowsMultipleUsers(account)) {
      return NextResponse.json(
        {
          error:
            'Starter Bot includes one business user. Upgrade or ask the super admin to enable a package with staff accounts before inviting members.',
        },
        { status: 403 }
      );
    }
